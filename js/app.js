 ...
 * ============================
 * Validaciones, cálculos y modales unificados
 * (versión simplificada y ordenada)
 */
...
// email tiene @ y . en posiciones válidas
...
const bindModalHandlers = (overlay) => {
  ...
  document.addEventListener("keydown", (e) => { if (e.key === "Escape" && overlay.style.display === "grid") hide(); });
...
const showErrorAndClear = (el, msg) => {
  ...
  el.value = ""; el.focus(); el.classList.add("is-error");
...
const validateDates = (inEl, outEl) => {
  ...
  if (!inD || !outD) return showErrorAndClear(outEl, "Completá Check-in y Check-out.");
  if (inD < today) return showErrorAndClear(inEl, "La fecha de Check-in no puede ser anterior a hoy.");
  if (outD <= inD) return showErrorAndClear(outEl, "La fecha de Check-out debe ser posterior al Check-in.");
  return true;
};

const validateRoomQty = i => {
  const n = +i.value || 0;
  if (n < 0 || n > 9) return showErrorAndClear(i, "Cantidad inválida (0–9)."), 0;
  return n;
};
...
/* -- FILTRO POR SELECT “Room and guests” -- */
...
const handleSubmit = e => {
  ...
  filterRoomsByGuests();          // filtra al hacer Search
  updateSummary(inEl, outEl);     // y refresca el resumen
};

/* -- RESTORE desde localStorage al volver desde payment -- */
const restoreFromCheckout = () => {
  let d;
  try { d = JSON.parse(localStorage.getItem("sb_checkout") || "{}"); } catch { d = {}; }
  ...
  // Ajustar select guests en base a lo elegido
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* -- Continue → payment.html (listener DIRECTO sobre el botón) -- */
  const contBtn = document.querySelector(".resumen .btn-payment");
  contBtn?.addEventListener("click", handleContinue);

  /* -- Restaurar si hay sb_checkout (vuelta desde payment) -- */
  restoreFromCheckout();
});

/* == lógica de Continue separada, sin delegación global == */
const handleContinue = () => {
  ...
  if (!checkin || !checkout)
    return showModal("Completá Check-in y Check-out.", "Error");
  if (nights <= 0)
    return showModal("La fecha de Check-out debe ser posterior al Check-in.", "Error");
  if (totalRooms === 0)
...
document.addEventListener("click", e => {
  ...
  if (b) { e.preventDefault(); openRoomDetails(b); }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  f.addEventListener("submit", (e) => {
    ...
    if (!n || !l || !m) {
      return showModal("Completá todos los campos obligatorios.", "Error");
    }
    if (!isLetters(n) || !isLetters(l)) {
      return showModal("Nombre y Apellido: solo letras.", "Error");
    }
    if (!isEmailBasic(m)) {
      return showModal("Email inválido.", "Error");
    }
    if (p && !isDigits(p)) {
      return showModal("Teléfono: solo números.", "Error");
    }

    showModal("¡Mensaje enviado!", "OK");
    f.reset();
  });
});


/* ======= PAYMENT (validación simple) ======= */
document.addEventListener("DOMContentLoaded", () => {
  const f = document.getElementById("pay-form");
  if (!f) return;

  // --- resumen
  const d = JSON.parse(localStorage.getItem("sb_checkout") || "{}");
  if (!d.total) return (location.href = "booking.html");
  ...
  document.getElementById("sum-total").textContent = money(d.total);
  document.getElementById("sum-rooms").innerHTML = d.rooms.map(r =>
    `<div class="sum-row"><span>${r.qty} ${r.name}</span><strong>${money(r.qty * r.price * d.nights)}</strong></div>`
  ).join("");
  ...
  const namec = document.getElementById("name"); // Name on card
  ...
  // tarjeta: mantener dígitos y formatear 4-4-4-4
  card?.addEventListener("input", () => {
    const digits = keepDigits(card.value).slice(0, 19); // permite 13–19
    const groups = [];
    for (let i = 0; i < digits.length; i += 4) groups.push(digits.slice(i, i + 4));
    card.value = groups.join(" ").trim();
  });

  // expiry: mantener dígitos y auto "/" → MM/YY
  ...
  f.addEventListener("submit", (e) => {
    ...
    if (!n || !l || !m || !cn || !nc || !ex || !cv) {
      return showModal("Completá todos los campos requeridos.", "Error");
    }
    if (!isLetters(n) || !isLetters(l) || !isLetters(nc)) {
      return showModal("Los nombres deben tener solo letras.", "Error");
    }
    if (!isEmailBasic(m)) {
      return showModal("Email inválido.", "Error");
    }
    ...
    if (!isDigits(cn) || cn.length < 13 || cn.length > 19) {
      return showModal("Número de tarjeta inválido.", "Error");
    }
    if (!isExpiryMMYY(ex)) {
      return showModal("Fecha inválida (MM/YY).", "Error");
    }
    if (!isDigits(cv) || cv.length !== 3) {
      return showModal("CVV inválido (3 números).", "Error");
    }
